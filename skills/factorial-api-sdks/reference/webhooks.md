...
| `review_request_at` | string | no | The date and time the expensable was submitted for review. Null while it is a draft; set at submission and never cleared, so it is always present once an approval flow or a policy check runs (they start at submission). Compare it with `effective_on` to measure how late a document was submitted. |
