...
def sync_detailed(
    *,
    ...
    """Reads all Allowance stats
     ...
     Retrieves the employee time off counters for a specific allowance with a reference date. Read
    without employee_ids it pages by employee with a cursor: limit and meta.total count employees rather
    than stats, so a page carries every stat those employees have, and the page parameter is not
    supported there - follow meta.end_cursor instead.
    ...
    Args:
        ids (list[str] | Unset): A virtual ID for the allowance stat, composed of
            ...
            1/2/2023-10-01.
        employee_ids (list[str] | Unset): IDs of the employees to read allowance stats for, at
            most 25 per request. Leave it empty to read every employee you have access to. Naming an
            employee you cannot access returns an empty page rather than the stats for the others.
            Example: ['1'].
        ...
        simulate_policy_change_to_id (str | Unset): When provided, computes the stats as if the
            ...
            un-simulated read. Can only be used together with employee_ids, because a simulation needs
            every employee in the read to belong to the target policy's company - which a company-wide
            read cannot guarantee. Example: 1.
