...
def sync_detailed(
    *,
    ...
    Args:
        ids (list[str] | Unset): Return only the leaves with these ids. Example: ['1'].
        employee_ids (list[str] | Unset): Return only the leaves of these employees. Example:
            ['1'].
        leave_type_id (list[str] | Unset): Return only the leaves of these leave types. Despite
            the singular name it takes a list. Example: ['36'].
        to (str | Unset): Return only the leaves that had already started by this date (start date
            on or before it). Usable on its own; pair it with `from` for a window. Example:
            2028-09-30.
        from_ (str | Unset): Return only the leaves that had not finished before this date,
            meaning they end on or after it or have no end date at all. Usable on its own; pair it
            with `to` for a window. Example: 2028-09-01.
        ...
        approved (bool | Unset): Set to true for leaves that were approved, false for leaves that
            were rejected. Leave it unset for every leave whatever its decision. Combining `approved:
            true` with `include_pending: true` returns approved and still-pending leaves together.
            Example: True.
        include_pending (bool | Unset): Set to true to include leaves still awaiting a decision.
            On its own it returns ONLY the pending ones; alongside `approved: true` it returns
            approved and pending together. Example: True.
        ...
        type_is_workable (bool | Unset): Set to true for leaves whose leave type counts as worked
            time, false for those that do not. Leave it unset to get both.
        type_is_payable (bool | Unset): Set to true for leaves whose leave type is paid, false for
            unpaid ones. Leave it unset to get both.
