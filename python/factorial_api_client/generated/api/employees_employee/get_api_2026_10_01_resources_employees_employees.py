...
from ...models.get_api_20261001_resources_employees_employees_response_200 import (
...
) -> GetApi20261001ResourcesEmployeesEmployeesResponse200 | None:
    if response.status_code == 200:
        response_200 = GetApi20261001ResourcesEmployeesEmployeesResponse200.from_dict(
            response.json()
        )
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GetApi20261001ResourcesEmployeesEmployeesResponse200]:
    return Response(
...
def sync_detailed(
    *,
    ...
    Args:
        ids (list[str] | Unset): Return only the employees with these ids. Ids outside what the
            reader may see are dropped, rather than failing the whole read. Example: ['1', '2', '3'].
        ...
        legal_entity_ids (list[str] | Unset): Return only the employees whose contract is under
            these legal entities. Example: ['1', '2'].
        company_identifier (str | Unset): filter by employee company identifier. Example:
            bb9d281e.
        only_active (bool): Set to true to keep only the employees not terminated as of today:
            those with no termination date, or one still in the future. Passing false has no effect,
            and it is ignored altogether when `include_active_and_terminated` is set. Example: True.
        team_ids (list[str] | Unset): Return only the employees who belong to these teams.
            Example: ['1', '2'].
        location_ids (list[str] | Unset): Return only the employees whose default workplace is one
            of these locations. Example: ['1', '2'].
        only_managers (bool): Set to true to return only the employees who have at least one
            direct report. Passing false has no effect. Example: True.
