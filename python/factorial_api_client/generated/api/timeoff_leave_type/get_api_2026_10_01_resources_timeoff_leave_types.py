...
def sync_detailed(
    *,
    ...
    Args:
        ids (list[str] | Unset): Return only the leave types with these ids. Example: ['1', '2'].
        company_ids (list[str] | Unset): Return only the leave types of these companies. Example:
            ['1', '2'].
        active (bool | Unset): Set to true for the leave types currently in use, false for the
            retired ones. Leave it unset to get both. Example: True.
        payable (bool | Unset): Set to true for the leave types that are paid, false for the
        ...
        employee_id (str | Unset): Return only the leave types the time-off policy in force for
            this employee grants. It REQUIRES `reference_date`, which says on which date to resolve
            that policy: without it the read raises. Example: 1.
        reference_date (str | Unset): The date on which to resolve the employee's time-off policy.
            Only meaningful alongside `employee_id`, and mandatory whenever that is given. Example:
            2024-08-22.
