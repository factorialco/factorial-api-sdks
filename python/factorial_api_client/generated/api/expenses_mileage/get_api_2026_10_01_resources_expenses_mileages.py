...
from ...models.get_api_20261001_resources_expenses_mileages_response_200 import (
...
) -> GetApi20261001ResourcesExpensesMileagesResponse200 | None:
    if response.status_code == 200:
        response_200 = GetApi20261001ResourcesExpensesMileagesResponse200.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GetApi20261001ResourcesExpensesMileagesResponse200]:
    return Response(
...
def sync_detailed(
    *,
    ...
    Args:
        ids (list[str] | Unset): Return only the mileage records with these ids. Example: ['1'].
        expenses_expensable_ids (list[str] | Unset): Return only the mileage records attached to
            these expensables. A mileage record is the detail record of an expensable, so this is the
            usual way to reach it from the header. Example: ['1'].
        include_manual_drafts (bool):
        employee_ids (list[str] | Unset): Return only the mileage records of these employees.
            Example: ['1'].
        external_authorization_ids (list[str] | Unset):
        card_ids (list[str] | Unset): Return only the mileage records paid with these cards.
            Example: ['1'].
        card_payment_ids (list[str] | Unset): Return only the mileage records raised from these
            card payments. Example: ['1'].
        include_attachments (bool):
        from_ (str | Unset): Start of the window of travel dates (`effective_on`) to return,
            inclusive. It only takes effect together with `to`: given on its own it is ignored.
            Example: 2024-01-01T00:00:00Z.
        to (str | Unset): End of the window of travel dates (`effective_on`) to return, exclusive.
            It only takes effect together with `from`. Example: 2024-02-01T00:00:00Z.
        dispute_ids (list[str] | Unset): Return only the mileage records under these payment
            disputes. Example: ['1'].
