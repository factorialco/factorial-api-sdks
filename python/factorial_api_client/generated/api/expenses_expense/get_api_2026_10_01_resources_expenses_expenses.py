...
from ...models.get_api_20261001_resources_expenses_expenses_response_200 import (
...
) -> GetApi20261001ResourcesExpensesExpensesResponse200 | None:
    if response.status_code == 200:
        response_200 = GetApi20261001ResourcesExpensesExpensesResponse200.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GetApi20261001ResourcesExpensesExpensesResponse200]:
    return Response(
...
def sync_detailed(
    *,
    ...
    Args:
        ids (list[str] | Unset): Return only the expenses with these ids. Example: ['1'].
        expenses_expensable_ids (list[str] | Unset): Return only the expenses attached to these
            expensables. An expense is the detail record of an expensable, so this is the usual way to
            reach it from the header. Example: ['1'].
        include_manual_drafts (bool): Whether to include manual drafts Example: True.
        employee_ids (list[str] | Unset): Return only the expenses of these employees. Example:
            ['1'].
        external_authorization_ids (list[str] | Unset): The ids of the external authorizations to
            filter by
        card_ids (list[str] | Unset): Return only the expenses paid with these cards. Example:
            ['1'].
        card_payment_ids (list[str] | Unset): Return only the expenses raised from these card
            payments. Example: ['1'].
        ...
        to (str | Unset): End of the window of expense dates (`effective_on`) to return,
            exclusive. It only takes effect together with `from`. Example: 2024-02-01T00:00:00Z.
        dispute_ids (list[str] | Unset): Return only the expenses under these payment disputes.
            Example: ['1'].
