...
from ...models.get_api_20251001_resources_project_management_project_tasks_due_status import (
    GetApi20251001ResourcesProjectManagementProjectTasksDueStatus,
)
from ...models.get_api_20251001_resources_project_management_project_tasks_response_200 import (
...
def _get_kwargs(
    *,
    ...
    overdue: bool,
    search: str,
    due_status: GetApi20251001ResourcesProjectManagementProjectTasksDueStatus,
    client_ids: list[int],
) -> dict[str, Any]:
    ...
    json_task_ids: list[int] | Unset = UNSET
...
) -> GetApi20251001ResourcesProjectManagementProjectTasksResponse200 | None:
    if response.status_code == 200:
        response_200 = GetApi20251001ResourcesProjectManagementProjectTasksResponse200.from_dict(
            response.json()
        )
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[GetApi20251001ResourcesProjectManagementProjectTasksResponse200]:
    return Response(
...
def sync_detailed(
    *,
    ...
    Args:
        ids (list[int]): Retrieve only the projects tasks that matches the ids passed in the
            request. Example: [314159, 271828].
        project_ids (list[int]): Retrieve only the projects tasks from the projects that matched
            the ids passed in the request. Example: [314159, 271828].
        subproject_ids (list[int]): Retrieve only the projects tasks from the subprojects that
        ...
        due_status (GetApi20251001ResourcesProjectManagementProjectTasksDueStatus): Retrieve only
            the project tasks that their due status match with the content passed as argument.
            Example: due_in_future.
        client_ids (list[int]): Retrieve only the projects tasks from the clients that matched the
