...
module F
  class LocationsLocation < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `F::LocationsLocation` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `F::LocationsLocation`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] main Value to be assigned
    def main=(main)
      if main.nil?
        fail ArgumentError, 'main cannot be nil'
      end
    ...
    def hash
      [id, company_id, name, timezone, country, state, city, address_line_1, address_line_2, postal_code, phone_number, main, latitude, longitude, radius, siret].hash
    end
