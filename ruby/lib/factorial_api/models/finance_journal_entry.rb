...
module F::Api
  class FinanceJournalEntry < ApiModelBase
    # Journal entry ID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `F::Api::FinanceJournalEntry` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `F::Api::FinanceJournalEntry`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      source_type_validator = EnumAttributeValidator.new('String', ["bank_transaction", "invoice", "reconciliation", "finance_reconciliation", "account", "expense", "payroll_result", "journal_entry"])
    ...
    def source_type=(source_type)
      validator = EnumAttributeValidator.new('String', ["bank_transaction", "invoice", "reconciliation", "finance_reconciliation", "account", "expense", "payroll_result", "journal_entry"])
    ...
    def hash
      [id, number, published_at, type, source_id, source_type, reference_date, description, legal_entity_id, external_id, status, updated_at].hash
    end
