...
module F::Api
  class CompaniesLegalEntitiesPostRequest < ApiModelBase
    # company identifier
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `F::Api::CompaniesLegalEntitiesPostRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `F::Api::CompaniesLegalEntitiesPostRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [company_id, country, legal_name, currency, tin, city, state, postal_code, address_line_1, address_line_2].hash
    end
