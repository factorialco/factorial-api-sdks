...
module F::Api
  class AtsEvaluationForm < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `F::Api::AtsEvaluationForm` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `F::Api::AtsEvaluationForm`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'based_on_id')
        self.based_on_id = attributes[:'based_on_id']
      end
    ...
    def hash
      [id, company_id, ats_job_posting_id, name, based_on_id, questions, created_at, updated_at].hash
    end
