...
module F::Api
  class AttendanceEditTimesheetRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `F::Api::AttendanceEditTimesheetRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `F::Api::AttendanceEditTimesheetRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, approved, request_type, employee_id, workable, clock_in, clock_out, location_type, reason, attendance_shift_id, time_settings_break_configuration_id, observations, date, reference_date].hash
    end
