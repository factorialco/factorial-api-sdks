...
BASE_URL = ENV["FACTORIAL_BASE_URL"].to_s.empty? ? "https://api.factorialhr.com" : ENV["FACTORIAL_BASE_URL"]
CLIENT_ID = ENV["FACTORIAL_OAUTH_CLIENT_ID"]
CLIENT_SECRET = ENV["FACTORIAL_OAUTH_CLIENT_SECRET"]
REDIRECT_URI = ENV["FACTORIAL_OAUTH_REDIRECT_URI"].to_s.empty? ? BASE_URL : ENV["FACTORIAL_OAUTH_REDIRECT_URI"]
...
def post_token(**params)
  ...
  body = JSON.parse(response.body) rescue { "raw" => response.body[0, 300] }

  unless response.is_a?(Net::HTTPSuccess)
    abort("ERROR: token endpoint returned HTTP #{response.code}: #{body.inspect}")
  end
...
def show(token_response)
  ...
  if (claims = decode_claims(access_token))
    ...
    claims.each { |k, v| puts format("  %-12s %s", k, v.to_s[0, 80]) }
