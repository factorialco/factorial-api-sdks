...
/**
 * Emits a resource method that delegates to a generated SDK function.
 *
 * `<true>` pins the function's `ThrowOnError` type parameter, so the method's
 * static type matches the runtime: it resolves to `{ data, request, response }`
 * and rejects with a `FactorialApiError` instead of resolving to
 * `{ data, error }`. The parameter stays as optional (or required) as the
 * generated function makes it.
 */
function delegatingMethod(name: string, fnName: string): string {
  return `  ${name}: typeof ${fnName}<true> = (options) => ${fnName}<true>({ client: this._client, ...options });`;
}
...
function generateResourceClass(ns: string, resource: string, entries: Endpoint[]): string {
  ...
  if (list) {
    ...
    lines.push(delegatingMethod("list", list.fnName));
    lines.push("");
    lines.push(`  /**`);
    ...
    lines.push(`   */`);
    lines.push(`  paginate(options?: Parameters<typeof ${list.fnName}<true>>[0] & { limit?: number; maxItems?: number }) {`);
    lines.push(`    const { maxItems, limit, ...rest } = options ?? {};`);
    lines.push(`    return paginate(`);
    lines.push(`      // \`limit\`/\`after_id\` work at runtime but are absent from the spec's query type.`);
    lines.push(`      // eslint-disable-next-line @typescript-eslint/no-explicit-any`);
    lines.push(`      (params) => ${list.fnName}<true>({ client: this._client, ...rest, query: { ...(rest as any).query, ...params } } as any),`);
    lines.push(`      { maxItems, limit },`);
    lines.push(`    );`);
    lines.push(`  }`);
    lines.push("");
    lines.push(`  /**`);
    ...
    lines.push(`   */`);
    lines.push(`  all(options?: Parameters<typeof ${list.fnName}<true>>[0] & { limit?: number; maxItems?: number }) {`);
  ...
  if (create) {
    ...
    lines.push(delegatingMethod("create", create.fnName));
    lines.push("");
  }
  if (getOne) {
    lines.push(`  /** Reads a single ${resource} record */`);
    lines.push(delegatingMethod("get", getOne.fnName));
    lines.push("");
  }
  if (update) {
    lines.push(`  /** Updates a ${resource} record */`);
    lines.push(delegatingMethod("update", update.fnName));
    lines.push("");
  }
  if (del) {
    lines.push(`  /** Deletes a ${resource} record */`);
    lines.push(delegatingMethod("delete", del.fnName));
    lines.push("");
  }
  for (const ca of custom) {
    lines.push(`  /** ${ca.method} */`);
    lines.push(delegatingMethod(ca.method, ca.fnName));
    lines.push("");
  }
